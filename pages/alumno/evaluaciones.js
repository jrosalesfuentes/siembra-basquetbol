...
export default function EvaluacionesAlumno() {
  ...
  useEffect(() => {
    const cargar = async () => {
      ...
      const [{ data: evals }, { data: antro }] = await Promise.all([
        supabase.from('evaluaciones').select('*').eq('alumno_id', al.id).order('fecha', { ascending: false }),
        supabase.from('evaluaciones_antropometricas').select('*').eq('alumno_id', al.id).order('fecha', { ascending: false })
      ])
  ...
  const evalsFiltradas = filtro === 'antropometría' ? antropometrias : evaluaciones.filter(e => e.tipo === filtro)
    ...
    <Layout rol="alumno">
      ...
      <div className="card">
        {loading ? <p className="text-center text-gray-400 py-8">Cargando...</p> :
          evalsFiltradas.length === 0 ? <p className="text-center text-gray-400 py-8 capitalize">Sin evaluaciones de {filtro} aún</p> :
          <div className="space-y-3">
            {filtro === 'antropometría' ? evalsFiltradas.map(ev => {
                ...
                <div key={ev.id} className="flex items-center gap-3 p-3 rounded-lg bg-gray-50">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-gray-800">
                      Peso: {ev.peso_kg} kg · Talla: {ev.talla_cm} cm
                    </p>
                    {ev.imc && <p className="text-xs text-gray-500">IMC: <strong>{ev.imc}</strong> — {claseIMC?.label}</p>}
